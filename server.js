...
// チャネリングテキスト生成API（NGフィルター追加）
app.post('/api/generate-channeling', async (req, res) => {
  console.log('🚀🚀🚀 API呼び出し確認 🚀🚀🚀');
  try {
    ...
    console.log('チャネリング生成リクエスト:', question);
    ...
    while (attempts < maxAttempts) {
      ...
      console.log(`チャネリング生成完了 (試行${attempts + 1})`);
    ...
    console.error('チャネリング生成エラー:', error);
    res.status(500).json({ 
...
app.post('/api/interpret-words', async (req, res) => {
  try {
    ...
    console.log('ワード読み解きリクエスト:', { originalQuestion, receivedWords });
...
これらの言葉の意味を読み解いてください。元の質問との関連性や、言葉に込められた深いメッセージを美しく解釈してください。`
      ...
      console.log(`ワード読み解き完了 (試行${attempts + 1})`);
...
// サーバー起動
app.listen(PORT, () => {
  console.log(`🆕 新しいサーバーがポート${PORT}で起動しました`);
  console.log(`ヘルスチェック: http://localhost:${PORT}/health`);
  console.log('利用可能なAPI:');
  console.log('- POST /api/generate-channeling (チャネリング生成) ✅NGフィルター 🆕新版');
  console.log('- POST /api/generate-audio (音声生成)');
  console.log('- POST /api/interpret-words (ワード読み解き) ✅NGフィルター');
});
